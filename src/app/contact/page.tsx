...
export default function ContactPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    address: "",
    service: SERVICE_CATEGORIES[0].label,
    message: ""
  });
    ...
    <>
      ...
      <Section background="main">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 w-full items-start">
          ...
          <aside className="lg:col-span-5 lg:sticky lg:top-28 bg-card p-8 md:p-10 rounded-2xl border border-border hover:border-accent/40 transition-all duration-500 shadow-xl space-y-6">
            <div>
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={idx} className="flex items-start space-x-5 border-b border-border last:border-0 pb-4 last:pb-0">
                  ...
                  <div>
                    <strong className="text-[12px] uppercase tracking-wider text-body/60 block mb-0.5">{item.title}</strong>
          ...
          <div className="lg:col-span-7 bg-card p-8 md:p-10 rounded-2xl border border-accent/25 shadow-2xl">
            ...
            {isFormSuccess && (
              <div className="mb-5 p-4 bg-green-50 border border-green-200 text-green-700 rounded-2xl flex items-center gap-3 animate-fade-in-up">
            ...
            <form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="contact-name" className="text-[12px] uppercase tracking-wider font-semibold text-heading">Full Name *</label>
                <input
                  type="text"
                  id="contact-name"
                  required
                  ...
                  className="w-full px-4 py-3 rounded-2xl border border-border bg-section text-sm text-heading focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition-all font-medium"
                  placeholder="Enter your full name"
                />
              </div>
              <div className="space-y-1">
                <label htmlFor="contact-address" className="text-[12px] uppercase tracking-wider font-semibold text-heading">Address *</label>
                <input
                  type="text"
                  id="contact-address"
                  required
              ...
              <div className="space-y-1">
                <label htmlFor="contact-phone" className="text-[12px] uppercase tracking-wider font-semibold text-heading">Phone *</label>
                <input
                  type="tel"
                  id="contact-phone"
                  required
              ...
              <div className="space-y-1">
                <label htmlFor="contact-email" className="text-[12px] uppercase tracking-wider font-semibold text-heading">Email *</label>
                <input
                  type="email"
                  id="contact-email"
                  required
              ...
              <div className="space-y-1 md:col-span-2">
                <label htmlFor="contact-service" className="text-[12px] uppercase tracking-wider font-semibold text-heading">Service required *</label>
                <div className="relative">
                  <select
                    id="contact-service"
                    value={formData.service}
                    onChange={(e) => setFormData({ ...formData, service: e.target.value })}
                    className="w-full px-4 py-3 rounded-2xl border border-border bg-section text-sm text-heading focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition-all font-medium appearance-none"
                  >
                    {SERVICE_CATEGORIES.map((cat) => (
                      <option key={cat.slug} className="bg-white text-heading">{cat.label}</option>
                    ))}
              ...
              <div className="space-y-1 md:col-span-2">
                <label htmlFor="contact-message" className="text-[12px] uppercase tracking-wider font-semibold text-heading">Message</label>
                <textarea
                  id="contact-message"
                  rows={3}
                  value={formData.message}
                  onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                  className="w-full px-4 py-3 rounded-2xl border border-border bg-section text-sm text-heading focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition-all resize-none font-medium text-heading"
      ...
      <section className="py-12 bg-section border-t border-b border-border">
        <div className="container mx-auto px-4 md:px-8">
          <div className="rounded-2xl overflow-hidden shadow-2xl border border-accent/15 h-[360px] relative bg-card gold-glow">
            <iframe
