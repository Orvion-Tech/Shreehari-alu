...
const serviceDivisions = SERVICE_CATEGORIES.map((cat) => ({
  name: cat.label,
                ...
                {isServicesOpen && (
                  ...
                  >
                    <div className="bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,37,42,0.15)] border border-accent/25 p-5 grid grid-cols-12 gap-6 overflow-hidden relative">
                        ...
                        {serviceDivisions.map((service, index) => {
                          ...
                          return (
                            <LinkNext 
                              key={index} 
                              href={service.href}
                              className="group p-3 rounded-2xl hover:bg-accent-light/30 border border-transparent hover:border-accent/15 transition-all duration-300 flex gap-3"
                            >
                              ...
                              <div className="space-y-0.5">
                                ...
                                <p className="text-[12px] text-body/80 leading-relaxed font-light line-clamp-2">
                                  {service.desc}
                                </p>
                              </div>
                            </LinkNext>
                      ...
                      <div className="col-span-4 bg-[#032326] rounded-2xl p-4 border border-accent/20 relative overflow-hidden flex flex-col justify-between text-white text-left z-10 shadow-inner">
                        <div className="absolute inset-0 cad-grid opacity-20 pointer-events-none" />
                        
                        <div className="space-y-2">
                          <span className="px-2 py-0.5 rounded bg-accent/10 border border-accent/20 text-[11px] font-mono text-accent uppercase tracking-wider inline-block">
                            Architectural Spec
                          </span>
                        ...
                        <a 
                          ...
                          target="_blank"
                          className="mt-4 w-full bg-accent hover:bg-accent/90 text-white py-2 px-3 rounded-lg text-[11px] font-mono tracking-wider uppercase font-bold text-center flex items-center justify-center gap-1.5 transition-colors border border-accent/20"
                        >
          ...
          >
            <X className="w-6 h-6" />
                  ...
                  {mobileMenuView === "services" && (
                    ...
                    >
                      <button 
                        ...
                        className="flex items-center gap-1.5 text-accent font-bold text-[12px] uppercase tracking-widest cursor-pointer hover:opacity-80 py-1"
                      >
                        ...
                        {serviceDivisions.map((service, index) => {
                          ...
                          return (
                            <LinkNext 
                              key={index}
                              href={service.href} 
                              className="group flex gap-3.5 p-3 rounded-2xl bg-primary/[0.02] hover:bg-accent-light/20 border border-primary/5 hover:border-accent/15 transition-all text-left"
                              onClick={() => setIsMobileMenuOpen(false)}
                            >
                              ...
                              <div className="space-y-0.5">
                                ...
                                <span className="block text-[12px] text-body leading-relaxed font-light line-clamp-2">
                                  {service.desc}
                                </span>
                              </div>
                            </LinkNext>
                      ...
                      <div className="p-4 bg-[#032326] rounded-2xl text-white relative overflow-hidden shadow-inner mt-4 border border-accent/15">
                        <div className="absolute inset-0 cad-grid opacity-10 pointer-events-none" />
                        <div className="relative z-10 space-y-2">
                          <span className="px-2 py-0.5 rounded bg-accent/15 border border-accent/25 text-[11px] font-mono text-accent uppercase tracking-wider inline-block">
                            Architectural Spec
                          </span>
                          ...
                          <a 
                            href="/Shreehari-Alu-Brochure.pdf" 
                            target="_blank"
                            className="mt-3 w-full bg-accent hover:bg-accent/90 text-white py-2 px-3 rounded-lg text-[11px] font-mono tracking-wider uppercase font-bold text-center flex items-center justify-center gap-1.5 transition-colors border border-accent/20"
                          >
              ...
              <div className="mt-8 pt-4 border-t border-primary/5 space-y-4 relative z-10">
                <div className="space-y-2 text-[12px] text-body/70 font-light">
                ...
                <div className="text-[11px] font-mono text-body/30 text-center uppercase tracking-widest">
